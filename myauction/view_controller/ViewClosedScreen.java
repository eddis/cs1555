...
public class ViewClosedScreen extends Screen {
	...
	private static PreparedStatement listClosedStatement = null;
	...
	public ViewClosedScreen(Session session) {
		...
		headerBox.setLine(0, " Previous (<)                Viewing Closed Auctions                         ");
		headerBox.setLine(1, "-----------------------------------------------------------------------------");
	...
	public void listProducts() {
		...
		try {
			...
			if (listClosedStatement == null) {
				listClosedStatement = session.getDb().prepareStatement(QueryLoader.loadQuery("myauction/queries/listClosed.sql"));
			}

			listClosedStatement.setString(1, session.getUsername());

			results = listClosedStatement.executeQuery();
